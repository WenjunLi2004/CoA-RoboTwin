    ...
    for i in range(episode_num):
        ...
        for j in range(0,left_gripper_all.shape[0]):
            ...
            if j != 0:
                ...
                # 创建8x8矩阵来容纳双臂数据
                gripper_matrix = np.zeros((8, 8), dtype=np.float32)
                # 左臂齐次变换矩阵 (4x4)
                ...
                # 右臂齐次变换矩阵 (4x4)
                right_transform = np.eye(4, dtype=np.float32)
                right_transform[:3, :3] = Rotation.from_quat(right_endpose[3:7]).as_matrix()  # 旋转
                right_transform[:3, 3] = right_endpose[:3]  # 位置
                gripper_matrix[4:8, 4:8] = right_transform
                gripper_matrix[7, 7] = right_gripper_all[j]  # 覆盖(7,7)位置存储夹爪状态
            else :
                gripper_matrix= np.eye(8, 8)
            # 以下变量未在训练中使用
